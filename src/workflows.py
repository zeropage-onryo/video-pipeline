...
from .db import _now, bootstrap_account_id, connect, own_table
...
CREATE TABLE IF NOT EXISTS workflows (
    ...
    -- A concept's canvas is a saved graph too (2026-08-28). It used to be
    -- rebuilt from the shot every time it opened and thrown away on exit,
    -- so the enhance output -- a paid Gemini call -- had to be re-run on
    -- every visit just to see it again. states_json is what each node
    -- PRODUCED; LiteGraph's serialize() carries config and position, not
    -- output.
    concept_id  BIGINT,
    shot_n      INTEGER,
    states_json TEXT,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_wf_shot
    ON workflows (concept_id, shot_n) WHERE concept_id IS NOT NULL;
"""
...
def init(dsn=None) -> None:
    with connect(dsn) as conn:
        conn.execute(SCHEMA)
        # tenancy: the FK to accounts(id) and the claim of any unowned row
...
def save_shot_graph(concept_id: int, shot_n: int, graph: dict,
    ...
    with connect(dsn) as conn:
        row = conn.execute(
            "SELECT id FROM workflows WHERE concept_id = %s AND shot_n = %s "
        ...
        if row:
            ...
            if payload[1] is None:
                conn.execute(
                    "UPDATE workflows SET updated_at = %s, graph_json = %s, "
                    "seed_hash = %s WHERE id = %s AND account_id IS NOT DISTINCT FROM %s",
                    (now, payload[0], seed_hash, row["id"], account_id))
            else:
                conn.execute(
                    "UPDATE workflows SET updated_at = %s, graph_json = %s, "
                    "states_json = %s, seed_hash = %s WHERE id = %s AND account_id IS NOT DISTINCT FROM %s",
...
def get_shot_graph(concept_id: int, shot_n: int, dsn=None, *,
                   account_id: Optional[int]) -> Optional[dict]:
    """The saved canvas for one shot, or None to build a fresh one."""
    with connect(dsn) as conn:
        row = conn.execute(
            "SELECT * FROM workflows WHERE concept_id = %s AND shot_n = %s "
            "AND account_id IS NOT DISTINCT FROM %s",
            (concept_id, shot_n, account_id),
        ).fetchone()
    if row is None:
...
def delete_shot_graphs(concept_id: int, dsn=None, *,
    ...
    with connect(dsn) as conn:
        cursor = conn.execute(
            "DELETE FROM workflows WHERE concept_id = %s AND account_id IS NOT DISTINCT FROM %s",
...
def create_workflow(name: str, graph: dict, brand: Optional[str] = None,
    ...
    with connect(dsn) as conn:
        row = conn.execute(
            "INSERT INTO workflows (created_at, updated_at, brand, name, "
            "graph_json, account_id) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id",
...
def update_workflow(workflow_id: int, name: Optional[str] = None,
                    graph: Optional[dict] = None, dsn=None, *,
...
def delete_workflow(workflow_id: int, dsn=None, *,
                    account_id: Optional[int]) -> bool:
    with connect(dsn) as conn:
        cursor = conn.execute(
            "DELETE FROM workflows WHERE id = %s AND account_id IS NOT DISTINCT FROM %s",
...
def list_workflows(brand: Optional[str] = None, dsn=None, *,
    ...
    query = "SELECT * FROM workflows WHERE concept_id IS NULL AND account_id IS NOT DISTINCT FROM %s"
    params: tuple = (account_id,)
    if brand:
        query += " AND (brand = %s OR brand IS NULL)"
...
def get_workflow(workflow_id: int, dsn=None, *,
    ...
    with connect(dsn) as conn:
        raw = conn.execute(
            "SELECT * FROM workflows WHERE id = %s AND account_id IS NOT DISTINCT FROM %s",
...
def seed_default(dsn=None) -> Optional[int]:
    ...
    return create_workflow("Prompt enhancement", default_template(),
                           brand=None, dsn=dsn, account_id=owner)
