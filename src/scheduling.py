...
def add_post(video_ref: str, caption: Optional[str], publish_at: str,
    ...
    with connect(db_path) as conn:
        row = conn.execute(
            ...
            "platform, publish_at) VALUES (%s, %s, %s, %s, %s) RETURNING id",
...
def due_posts(now: Optional[str] = None, db_path=None) -> list[dict[str, Any]]:
    ...
    with connect(db_path) as conn:
        rows = conn.execute(
            ...
            "WHERE status = 'planned' AND publish_at <= %s "
...
def mark_status(post_id: int, status: str, media_id=None, error=None,
    ...
    with connect(db_path) as conn:
        conn.execute(
            # the casts: psycopg cannot infer a bare parameter's type inside
            # CASE WHEN, and Postgres refuses to guess
            "UPDATE scheduled_posts SET status = %s, "
            "media_id = COALESCE(%s, media_id), error = %s, "
            "posted_at = CASE WHEN %s::text = 'posted' THEN %s::text ELSE posted_at END "
            "WHERE id = %s",
...
def _posted_in_last_day(now: str, db_path=None) -> int:
    ...
    with connect(db_path) as conn:
        row = conn.execute(
            ...
            "WHERE status = 'posted' AND posted_at > %s",
            (cutoff,),
        ).fetchone()
        return row["n"]
