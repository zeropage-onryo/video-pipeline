...
import { held, usable, firstUsable, defaultPick, pickFor, withModel, fitSeconds, planFor, approveText, chipText, legalDuration, specOf } from '../src/lib/render-choice.ts';
...
test('fitSeconds fits a window UP to the model (timeline.fit_seconds twin)', () => {
  const kling = specOf(renderers, 'fal', 'kling').duration;
  assert.equal(fitSeconds(kling, 3), 5);
  assert.equal(fitSeconds(kling, 7), 10);
  assert.equal(fitSeconds(kling, 14), 10);
  assert.equal(fitSeconds(specOf(renderers, 'fal', 'ltx').duration, 3.2), 4);
  assert.equal(fitSeconds(specOf(renderers, 'veo', 'veo-3').duration, 3), 8);
  assert.equal(legalDuration(kling, 7), false);
  assert.equal(legalDuration(specOf(renderers, 'fal', 'ltx').duration, 7), true);
});

test('the button carries shot count and price; rendered shots are skipped', () => {
  ...
  const plan = planFor(spec, pick, parts);
