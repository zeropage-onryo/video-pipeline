...
from src import db, format_feed, shootgen, winners
...
def test_empty_library_returns_evergreens_in_order(tmp_db):
    got = format_feed.rank_formats(path=tmp_db)
...
def test_winners_float_a_proven_format_to_the_top(tmp_db):
    ...
    for _ in range(3):
        winners.add("veo", "one continuous slow push in toward the door, dolly in",
                    note="slow push worked", verdict="worked", path=tmp_db)
    ranked = format_feed.rank_formats(path=tmp_db)
...
def test_spice_is_prepended_on_top(tmp_db):
    ranked = format_feed.rank_formats(path=tmp_db, spice=["liminal pool loops"])
...
def test_never_raises_on_a_broken_store(monkeypatch):
    ...
    got = format_feed.rank_formats(path="whatever")
    assert got == list(shootgen.ZEROPAGE_FORMATS)


def test_limit_caps_the_menu(tmp_db):
    ranked = format_feed.rank_formats(path=tmp_db, limit=3)
    assert len(ranked) == 3
