...
def seed_winner_and_dud(db_path, posted_at="2026-06-01"):
    ...
    """
    winner_id = db.add_video("Winner", "youtube", posted_at, path=db_path)
    dud_id = db.add_video("Dud", "youtube", posted_at, path=db_path)
...
def test_run_auto_promotes_candidates_that_clear_the_bar(tmp_db, no_rag_network, monkeypatch):
    ...
    winner_id = db.add_video("Blowout", "youtube", "2026-06-01", path=tmp_db)
    mid_id = db.add_video("Mid", "youtube", "2026-06-01", path=tmp_db)
    dud_id = db.add_video("Dud", "youtube", "2026-06-01", path=tmp_db)
    db.record_metrics(winner_id, views=100_000, captured_at="2026-06-08T00:00:00", path=tmp_db)
    db.record_metrics(mid_id, views=2_000, captured_at="2026-06-08T00:00:00", path=tmp_db)
    db.record_metrics(dud_id, views=1_000, captured_at="2026-06-08T00:00:00", path=tmp_db)
