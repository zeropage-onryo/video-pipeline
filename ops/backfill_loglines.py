"""Give every scene concept a logline that FITS the card's one line.

Two kinds of row need one (2026-08-31):

  * written before the writer was asked for a logline at all, so the
    board falls back to preprod.derive_logline and shows a clause off
    the BEATS block -- scannable, but not a summary;
  * carrying a logline from the singular writer, which was never given
    a length budget: 93-148 characters, so the card ellipsised most of
    it away and the line read as cut off.

Both get one asked off the prompt the concept already has, under the
same LOGLINE_RULES the writer now follows. Costs one small text call
per rewritten concept and touches nothing else -- the prompt is never
rewritten, and a row whose logline already fits is skipped, so
...
def needs_one(concept: dict) -> bool:
    ...
    "Needs one" means the card cannot print it whole: no logline at all,
    or one over the budget concept_summary would trim. Asking whether
...
def main(write: bool, account_id: Optional[int] = None) -> int:
    ...
    if not wanted:
        print("every concept already has a logline that fits")
        return 0

    lines = shootgen.write_loglines(wanted, gemini_client=client)
    touched = 0
