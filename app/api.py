...
def me(request: Request, account_id: int = Depends(auth.current_account_id)):
    """Who is signed in, and which brands they may switch between.

    Composed from what auth and accounts already know rather than stored
    anywhere: `auth.current_user` is the Supabase profile mirror,
    `accounts.memberships` is the real membership list, and the active
    account is whatever `auth.current_account` resolved for this request.

    `accounts` is the list the brand switcher offers, and it is
    deliberately the MEMBERSHIP list rather than every account on the
    box: the `brand` cookie is a preference among accounts you belong to,
    never a grant (app/main.py's POST /brand/{name} re-checks anyway).
    """
    user = auth.current_user(request) or {}
    active = auth.current_account(request) or {}
    rows = accounts.memberships(user.get("id")) if user.get("id") else []
    known = [{"slug": r["slug"],
              "display_name": r.get("display_name") or r["slug"],
              "role": r.get("role") or "member"}
             for r in rows]
    slug = active.get("slug")
    current = next((a for a in known if a["slug"] == slug), None) or {
        "slug": slug or "",
        "display_name": active.get("display_name") or slug or "",
        "role": "member",
    }
    return {
...
def concept_detail(concept_id: int, account_id: int = Depends(auth.current_account_id)):
    ...
    return {**card, "duration": concept.get("duration"),
            ...
            # and what one clip would cost. spend_ok is the key again --
            # pressing the button IS the approval (2026-09-09).
