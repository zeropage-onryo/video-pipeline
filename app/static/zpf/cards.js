...
/* READY / CHECK / BLOCKED.
   ...
   NOT the prompt gate's verdict: prompt_scores (score, passed, reason)
   is not on this payload -- only the MCP `idea` tool joins it -- so the
   dot is derived from what the card does carry. BLOCKED is the reference
   gate's own fact (no refs, preprod.reference_gate). CHECK is anything a
   person should read first: validate_concept's warnings, or the advisory
   verdict the night wrote into park_reason ("advisory: prompt gate
   4/10 — ..."). READY is the absence of both, and says only that. */
export function gateOf(c) {
  ...
  if (warnings.length || advisory) {
    const why = advisory ? c.park_reason
