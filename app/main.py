...
def evals_page(request: Request):
    ...
    the signed-in product surface. The page is a shell; its data and
    actions come from the same /api/evals/* and /api/retrieve endpoints
    the old view used, so nothing is reimplemented here. Those routes
    require a session, and the page says so when it gets a 401 rather
